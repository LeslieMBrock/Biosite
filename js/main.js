...
function initLanguageToggle() {
	...
	const navToggleButton = document.getElementById("menu-toggle");
	if (!toggleButton) {
		return;
	}
	...
	const applyLanguageState = () => {
		...
		const languageLabel = isArabic
			? toggleButton.dataset.labelAr || "تبديل اللغة"
			: toggleButton.dataset.labelEn || "Toggle language";
		toggleButton.setAttribute("aria-label", languageLabel);

		if (navToggleButton) {
			const navLabel = isArabic
				? navToggleButton.dataset.labelAr || "تبديل التنقل"
